...
fn main() {
    ...
    let safe_report_count = reports
        ...
        .count();
    let tolerable_report_count = reports
        .iter()
        .filter(|report| is_report_tolerable(report))
        .count();
    println!(
        "Advent of Code 2024 - Day 2\n[Part 1] Safe Reports: {}\n[Part 2] Tolerable Reports: {}",
        safe_report_count, tolerable_report_count
    );
}
...
fn is_report_safe(reactor_levels: &[u32]) -> bool {
    ...
    reactor_levels.array_windows().all(|&[lhs, rhs]| {
        (1..4).contains(&lhs.abs_diff(rhs)) && if is_increasing { lhs < rhs } else { lhs > rhs }
    })
}
...
fn is_report_tolerable(reactor_levels: &[u32]) -> bool {
    ...
    (0..reactor_levels.len()).any(|i| {
        ...
        let potential_tolerable_report: Vec<_> = reactor_levels[0..i]
            .iter()
            .chain(&reactor_levels[i + 1..])
            .copied()
            .collect();
